* 进程管理：Linux上的**线程**用`task_struct`描述，也是内核的调度单元。将多个`task_struct`合并为一组(通过该结构内部的组id字段)再来描述一个**进程**。因此，Linux上的线程，也称**轻量级进程**。
  ...
  * 进程间通信
    ...
    * `XSI IPC`(由System V引入) 
      ...
      * 信号量(还有`POSIX`版本)：`PV`操作，用于实现进程同步和互斥
      * 共享内存(还有`POSIX`版本，利用的是`mmap`)：速度快，两进程虚拟内存映射到同一物理内存
    * 信号`kill`：异步通信机制
    * `socket`：不同主机进程通信
