...
* inline
  ...
  * inline放在函数声明/定义之前，将函数声明为内联函数，在类内定义的函数默认内联。内联函数会向编译器发出在函数调用点展开的请求，但编译器完全可以忽略。
  * inline放在全局变量定义之前，将变量视为内联变量。
  * inline目前最直观的作用是告诉链接器在链接的时候只保留函数/变量一份定义，因此inline修饰的函数/变量一般是为了放在头文件。
...
* auto和decltype：主要用于模板
  ...
  * 使用auto 需要注意的地方
    ...
    * auto作用于数组时，得到的是指针类型
    ...
    > auto可用于函数的尾置返回，c++14后可以直接返回auto
